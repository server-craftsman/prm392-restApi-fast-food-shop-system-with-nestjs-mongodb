...
export class ProductsDocumentRepository implements ProductRepository {
    ...
    async findManyWithPagination({
        ...
        if (filterOptions?.countryOfManufacture) {
            query.countryOfManufacture = { $regex: filterOptions.countryOfManufacture, $options: 'i' };
        }
        ...
        if (filterOptions?.createdAt) {
            query.createdAt = { $gte: filterOptions.createdAt, $lte: filterOptions.createdAt };
        }

        if (filterOptions?.updatedAt) {
            query.updatedAt = { $gte: filterOptions.updatedAt, $lte: filterOptions.updatedAt };
        }
    ...
    async findByIds(ids: Product['id'][]): Promise<Product[]> {
        ...
        const products = await this.productsModel.find({ _id: { $in: objectIds } }).exec();
        return products.map(ProductMapper.toDomain);
    }
    ...
    async update(
        ...
        const updated = await this.productsModel.findByIdAndUpdate(
            id,
            { $set: payload },
            { new: true }
        ).exec();
