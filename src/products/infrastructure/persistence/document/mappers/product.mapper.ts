...
export class ProductMapper {
  static toDomain(raw: ProductSchemaClass): Product {
    ...
    domainEntity.images =
      raw.images?.map((image) => FileMapper.toDomain(image)) || [];
  ...
  static toPersistence(domainEntity: Product): ProductSchemaClass {
    ...
    persistenceEntity.images =
      domainEntity.images?.map((image) => FileMapper.toPersistence(image)) ||
      [];
    ...
    persistenceEntity.storageInstructions =
      domainEntity.storageInstructions || '';
    ...
    persistenceEntity.countryOfManufacture =
      domainEntity.countryOfManufacture || '';
